  ...
  return (
    <div style={{ top: 12, zIndex: 10, padding: 14, borderRadius: 14, background: "#1a1a1a", marginBottom: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <div>
          <div style={{ fontSize: 12, opacity: .8 }}>지갑(USDT)</div>
          <div style={{ fontSize: 22, fontWeight: 800 }}>{fmtComma(wallet, 1)}</div>
        </div>
        <div>
          <div style={{ fontSize: 12, opacity: .8 }}>평가액</div>
      ...
      <div style={{ marginTop: 16, marginBottom: 8 }}>
          <div style={{ fontWeight: 700 }}>
            ...
            <span style={{ opacity: .7 }}>
              ({`총진입 : ${fmtComma(totalEntryNotional, 1)} USDT (${totalEntryPct.toFixed(1)}%)`})
            </span>
          </div>
        </div>

      {/* 포지션 테이블: 행마다 총 진입 USDT / 총 진입 % 컬럼 추가 (avg*qty 기준) */}
      <div>
        {rows.length === 0 ? (
          ...
          <div
            style={{
              display: "grid",
              gridTemplateColumns: "10ch 7ch 9ch 11ch 11ch 11ch 8ch 12ch 11ch",
              columnGap: 10,
              rowGap: 6,
              fontSize: 12
            }}
          >
            {/* 헤더 */}
            <div style={{ opacity: .65 }}>심볼</div>
            <div style={{ opacity: .65 }}>방향</div>
            ...
            <div style={{ opacity: .65, textAlign: "right" }}>진입 USDT</div>
            <div style={{ opacity: .65, textAlign: "right" }}>진입 %</div>
            ...
            {rows.map((r, i) => {
              ...
              return (
                <React.Fragment key={`${r.sym}-${r.side}-${i}`}>
                  ...
                  <div style={{ textAlign: "right", color: r.pnl >= 0 ? "#2fe08d" : "#ff6b6b", fontWeight: 700 }}>
                    {fmtComma(r.pnl, 2)}
                  </div>
                  <div style={{ textAlign: "right", color: (r.pnlPct ?? 0) >= 0 ? "#2fe08d" : "#ff6b6b" }}>
                    {r.pnlPct != null ? `${r.pnlPct.toFixed(2)}%` : "—"}
                  </div>
                  <div style={{ textAlign: "right" }}>{fmtComma(rowEntryUSDT, 1)}</div>
                    <div style={{ textAlign: "right" }}>{`${rowEntryPct.toFixed(1)}%`}</div>
                </React.Fragment>
              );
            })}
          </div>
