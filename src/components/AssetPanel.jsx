...
export default function AssetPanel({ asset, statsBySymbol }) {
  ...
  return (
    <div style={{ position: "sticky", top: 12, zIndex: 10, padding: 14, borderRadius: 14, background: "#1a1a1a", marginBottom: 14 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        ...
        <div><div style={{ fontSize:12, opacity:.8 }}>자산 평가</div><div style={{ fontSize:22, fontWeight:800 }}>{fmtComma(equity, 1)}</div></div>
      </div>
      ...
      <div style={{ marginTop: 12 }}>
        ...
        {rows.length === 0 ? (
          ...
          <div style={{ display:"grid", gridTemplateColumns:"10ch 7ch 9ch 11ch 11ch 11ch 8ch", columnGap:10, rowGap:6, fontSize:12 }}>
            ...
            <div style={{ opacity:.65, textAlign:"right" }}>진입가</div>
            ...
            {rows.map((r,i)=>(
              <React.Fragment key={`${r.sym}-${r.side}-${i}`}>
                ...
                <div style={{ textAlign:"right" }}>{fmtComma(r.qty,6)}</div>
                <div style={{ textAlign:"right" }}>{fmtComma(r.avg,4)}</div>
                <div style={{ textAlign:"right" }}>{r.px!=null?fmtComma(r.px,4):"—"}</div>
                <div style={{ textAlign:"right", color:r.pnl>=0?"#2fe08d":"#ff6b6b", fontWeight:700 }}>{fmtComma(r.pnl,4)}</div>
