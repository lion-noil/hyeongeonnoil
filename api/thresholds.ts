...
import { loadTradingConfig } from "./tradingConfig"; // 위에서 수정한 파일
...
async function getLatestFracBySymbol(
  ...
  const raw = await (redis as any).xrevrange(
    streamKey,
    "+",
    "-",
    { count: searchBack }
...
export default async function handler(
    ...
    return json(
      {
        retCode: -1,
        retMsg:
          "Env missing UPSTASH_REDIS_REST_URL/TOKEN",
      },
      500
    );
  }
  ...
  try {
    ...
    const nameParam =
      (searchParams.get("name") || "").trim(); // 네임스페이스
    const namespace = nameParam || "bybit";

    const crossLimitParam = parseInt(
      searchParams.get("cross_limit") || "0",
      10
    );
    const crossLimit = isNaN(crossLimitParam)
      ? 0
      : crossLimitParam; // 0이면 제한 없음

    if (!symbolParam)
      return json(
        { retCode: -1, retMsg: "symbol required" },
        400
      );

    const streamKey = `trading:${namespace}:OpenPctLog`;

    // 1) 네임스페이스별 trading:{name}:config 로드
    const cfg = await loadTradingConfig(
      redis,
      namespace
    );

    // 2) 최근 임계치(new, 0~1) — 네임스페이스별 스트림에서 검색
    const found = await getLatestFracBySymbol(
      redis,
      symbolParam,
      streamKey,
      500
